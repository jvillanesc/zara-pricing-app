package com.zara.princingapp.util;

/**
 * [Description].
 * <br/>
 * <b>Class</b>: {@link PriceNotExistException}<br/>
 * <b>Copyright</b>: &Copy; 2023 Zara INC;. <br/>
 * <b>Company</b>: Zara INC. <br/>
 *
 * @author Zara INC. <br/>
 *         <u>Service Provider</u>: Capilote SAC<br/>
 *         <u>Developed by</u>: <br/>
 *         <ul>
 *         <li>Jose Villanes C. (Capilote)</li>
 *         </ul>
 *         <u>Changes</u>:<br/>
 *         <ul>
 *         <li>Mar. 10, 2023 Creation class.</li>
 *         </ul>
 * @version 1.0
 */
public class PriceNotExistException extends RuntimeException {
  public PriceNotExistException() {
    super("Price does not exist.");
  }
}
